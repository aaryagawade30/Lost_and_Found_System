...
def login():
    if request.method == 'POST':
        ...
        cursor = mysql.connection.cursor(MySQLdb.cursors.DictCursor)
        # Fetch by email only, then verify the password with check_password_hash
        cursor.execute('SELECT * FROM users WHERE email = %s', (email,))
...
def add_item():
    ...
    if request.method == 'POST':
        ...
        cursor = mysql.connection.cursor(MySQLdb.cursors.DictCursor)
        cursor.execute(
            'INSERT INTO items (title, description, category, date_reported, location, image_path, user_id) '
